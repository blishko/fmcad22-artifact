...
echo "#########################################"
bash "${ARTIFACT_ROOT}/scripts/to_summary_table.sh" | column -t -s','

echo ""
echo "Showing full results for safe version of multi-phase benchmarks"
echo "#########################################"
bash "${ARTIFACT_ROOT}/scripts/to_table.sh" "${ARTIFACT_ROOT}/times/multi-phase_safe" | column -t -s','

echo ""
echo "Showing full results for unsafe version of multi-phase benchmarks"
echo "#########################################"
bash "${ARTIFACT_ROOT}/scripts/to_table.sh" "${ARTIFACT_ROOT}/times/multi-phase_unsafe" | column -t -s','
